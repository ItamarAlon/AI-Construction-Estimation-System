...
SYSTEM_PROMPT_SELECT_IDS = (
    ...
    "These are NOT measured by length. Tag their segments 'ignore (per-unit item, counted "
    "separately)' in the classification table. Get the actual count ONE of these ways:\n"
    "  - call 'count_outline_shapes_by_color' on the symbol's color (it merges multi-segment "
    "symbols into one item); or\n"
    "  - read the plan image and count visually.\n"
    "Never count by tallying table rows — one physical item is often several segments. "
    "If you cannot find a distinct symbol for a per-unit task, omit it.\n\n"

...
SYSTEM_PROMPT_SELECT_IDS = (
    ...
    "  - PARALLEL WALL FACES: walls in construction drawings are almost always drawn as TWO "
    "parallel lines of the same color — one for each face of the wall. These two lines are a "
    "SINGLE physical element. Assign BOTH faces to the SAME task. Never split the two parallel "
    "lines of one wall between two different tasks (e.g. one face to 'Building Wall' and the "
    "other face to 'Wall thickening'). If a segment is very close and parallel to another "
    "segment of the same color, they are the two faces of one wall — tag both to the same task. "
    "Assign all segments of a color on a given wall to whichever single task best matches the "
    "wall's label or the legend; do NOT divide them.\n"
